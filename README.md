...
- **[configs/claude-code-startup-context/](configs/claude-code-startup-context/)** — five always-loaded tool definitions removed from `~/.claude/settings.json`, and the method used to prove what they cost. Claude Code re-sends every tool definition on **every turn**; since 2.1.69 most built-ins are *deferred* (the prompt carries the name, the schema arrives on demand) but the largest one, `Artifact` at 67,389 bytes, is not. The README carries the calibration recipe (append a known block, read the token meter: **4.21 bytes per token**, linear from 3 KB to 61 KB), the two traps that cost real time, a per-tool table of what replaced each denied tool, and a script to count your own usage first. Includes the warning that earned it: a 519-call tool looked essential until the rule generating those calls turned out to be our own.
